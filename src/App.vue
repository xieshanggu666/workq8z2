...
// 采购 Tab 角标：运营看待审批 + 待入库（含部分入库），用户不提示
const purchaseBadge = computed(() =>
  store.role === 'operator' ? store.pendingPurchaseCount : 0)
...
.tab-badge.purchase { background: #ef6c00; box-shadow: 0 2px 6px rgba(239,108,0,0.5); }
